...
- **`src/logic/winnerFanfare.ts`** (nýtt, eftir ábendingu notanda) — TVÆR endurskrifanir eftir beina notanda-hlustun, engin þeirra alvöru upptaka/höfundarréttarvarið efni, í takt við sömu reglu og gildir annars staðar í appinu (sjá Spotify/YouTube ákvarðanirnar):
  1. Fyrsta útgáfa: tónræn (sveiflugjafar sem spiluðu stutt „fanfare"-stef) — notandi sagði hljóðin einfaldlega „hræðileg".
  2. Önnur útgáfa: viðvarandi síu-suð sem botnlag undir klappi/frópum — notandi sagði þetta hljóma eins og „rigning eða eitthvað suð", ekki fólk. Rótin: viðvarandi bandpass-síað hvítt suð ER í grunninn nákvæmlega það sem rigning/hvæs hljómar eins og — alvöru lófaklapp er í raun MARGIR AÐSKILDIR smellir ofan í hvorn annan, ekki samfelld suðræma.
  3. Núverandi (þriðja) útgáfa, beint eftir ábendingu notanda um „klapp klapp klapp... vúhú": botnlags-suðið tekið alveg út. Í staðinn: `addClap()` — stuttur (~45ms), bandpass-síaður (1,5–3,5kHz, þar sem alvöru lófaklapp einbeitir orku sinni) suðgususmellur með snarpri árás og hraðri deyfingu, notaður bæði stakt (nokkrir vel aðskildir smellir í upphafi, ~220ms á milli — bókstaflega „klapp... klapp... klapp") OG þétt (mörg tugir tilviljanakenndra smella síðar, sem klappflóð). `addWoohoo()` — sagtennt (sawtooth) sveiflugjafi sem rennur hratt upp („Voooo-"), heldur svo með léttu vibrato („-húú!") og deyfist — mun meira sannfærandi sem manns-óp en nokkurt síað suð, ein til tvær svona hrópanir blandaðar inn á milli klappanna. `master`-gain-hnúturinn fade-ar sjálfur inn (~50ms) og út (~0,5 sek) yfir alla lengdina, eins og beðið var um.
  - Byrjar SAMSTUNDIS þegar spuna-hreyfingin byrjar (ekki eftir að hún stöðvast) og dofnar út þegar lengri sprengingarlagið (`GLOW_MS`) er búið að klárast — spannar alla Winner reveal-röðina. `primeWinnerFanfare()` er áfram kölluð SAMSTUNDIS inni í „Final Results" smellinum sjálfum (á undan `await applyFinalRoundScores()`) svo `AudioContext`-inn sé þegar í gangi. Mistakist Web Audio af einhverjum ástæðum er þagað þunnu hljóði.
  - **Aukalagfæring fannst við prófun**: `playWinnerCheer()` skilar núna `stop()` falli sem WinnerRevealCard-effect-ið kallar í sinni EIGIN cleanup-fallun (við hliðina á núverandi `spinAnim.cancel()`) — annars myndi t.d. React StrictMode's tvöfalda dev-mode mount/cleanup/mount (eða bara alvöru snemmbúið „tap to skip") skilja eftir fagnaðarlætin keyrandi ofan í næstu hljóðupplifun. `stop()` þarf ekki að stöðva hvern einasta hljóðgjafa stakt — af því ALLIR tengjast í gegnum EITT `master`-gain, dugar að hreinsa áætluð gildi þess og setja í 0 til að þagga allt í einu.
- Staðfest með lifandi Playwright-prófun: fyrsta hljóðið heyrist samstundis (samstillt við smellinn, ekki eftir að spuninn stöðvast); sparse klöppin lenda á réttum, vel aðskildum tímapunktum (0/220/440ms); vúhú-hrópin lenda nákvæmlega á réttum stað í röðinni; þéttur fjöldi klapp-hljóðgjafa til staðar fyrir sjálft flóðið. Sjálf hljóðgæðin („er þetta í alvöru sannfærandi klapp/fagnaðarlæti?") er huglægt mat sem sjálfvirk próf ná ekki utan um — þarf lifandi hlustun.
