...
- **„Leave Game" á Results (`leaveGame(false)`) má ekki festa hópinn:** sá hnappur heldur viðkomandi í `players` (fyrir stigatöfluna) en skrifar núna líka `lobbyReady/{id}: true` fyrir sig um leið og hann fer — annars beið `finalizeRoundIfReady` (sem krefst `lobbyReady` frá ÖLLUM `players`) endalaust eftir einhverjum sem er farinn og kemur aldrei til með að ýta á „Go to lobby". Villan sem kom upp í alvöru: host fór úr leiknum á Results, hinir tveir ýttu á „Go to lobby" og sátu fastir á „Waiting for everyone to head back to the Lobby..." að eilífu, og gamli hostinn komst ekki einu sinni inn aftur (join-hliðið hleypir bara inn í `phase === 'lobby'`, sem náðist aldrei).
- **Reconnect með sama nafni** (`joinGame`): ef `phase !== 'lobby'` (umferð þegar hafin) og einhver reynir að joina með nafni sem passar (case-insensitive) við núverandi spilara í `players`, er það meðhöndlað sem ENDURTENGING — sama `playerId` er tekið upp aftur (þeirra fyrri lag/gisk/einkunnir, sem eru öll geymd á því ID-i, koma með) í stað þess að hafna með „That game has already started". Alveg nýtt nafn er samt áfram hafnað. Þetta er til viðbótar við `resumeSession()` (sem virkar bara ef `localStorage` lifir af — sama vafri/tæki) og grípur tilvikið þar sem einhver missir sambandið og þarf að byrja join-flæðið alveg upp á nýtt (nýtt tæki, hreinsað `localStorage`, o.s.frv.). Engin „presence"-staðfesting er til að vita hvort upprunalega session-ið sé raunverulega farið — nafnasamsvörun er sami trausts-grunnur og room-kóðinn sjálfur byggir á (ekkert innskráningarkerfi til að byrja með), í lagi fyrir vinahóp, ekki öryggisvörn.
