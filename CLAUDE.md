...
- **`src/logic/winnerFanfare.ts`** (nýtt, eftir ábendingu notanda): fyrsta útgáfa var tónræn (sveiflugjafar/oscillators sem spiluðu stutt „fanfare"-stef) — notandi prófaði og sagði hljóðin einfaldlega „hræðileg". Endurskrifað alveg frá grunni sem TILBÚIÐ (synthesized) áhorfenda-fagnaðarlæti/klapp í staðinn — hvorki alvöru upptaka né höfundarréttarvarið lag, í takt við sömu reglu og gildir annars staðar í appinu um að hýsa aldrei tónlist beint (sjá Spotify/YouTube ákvarðanirnar). Byggt eingöngu á Web Audio API-suði (`createBufferSource` + handahófskennt hvítt suð, ekki sveiflugjafar), þrjú lög ofan í hvort annað: viðvarandi sía-suð sem „mannmergð" botnlag (bandpass-sía, hægfara handahófs-magnbreyting svo það hljómi ekki eins og flatt hvæs), þéttur fjöldi stuttra klapp-hljóða (háþéttnisíuð suðgusa, ~16/sek af lengdinni) og nokkur „fagnaðaróp" (bandpass-sía sem sveiflast upp og niður í tíðni yfir suðgusu). `master`-gain-hnúturinn fade-ar sjálfur inn (~0,2 sek) og út (~0,5 sek) yfir alla lengdina, eins og notandi bað sérstaklega um. Byrjar SAMSTUNDIS þegar spuna-hreyfingin byrjar (ekki eftir að hún stöðvast) og dofnar út þegar lengri sprengingarlagið (`GLOW_MS`) er búið að klárast — spannar alla Winner reveal-röðina, ekki bara stutt stef í restina. `primeWinnerFanfare()` er áfram kölluð SAMSTUNDIS inni í „Final Results" smellinum sjálfum (á undan `await applyFinalRoundScores()`) svo `AudioContext`-inn sé þegar í gangi. Mistakist Web Audio af einhverjum ástæðum er þagað þunnu hljóði — aldrei nokkuð sem gæti brotið sjálfa Winner reveal-röðina.
- Staðfest með lifandi Playwright-prófun (tónræna útgáfan): `AudioContext` býr til nákvæmlega EINU sinni. Staðfest aftur eftir endurskrifun í áhorfenda-læti: fyrsta hljóðið heyrist ~190ms eftir smell (samstundis með spunanum, ekki eftir að hann stöðvast), og >120 hljóðgjafar (botnlag+klapp+fagnaðaróp) spilast yfir alla lengdina.
