...
**„Unmute" hnappurinn var of auðvelt að missa af (kom upp í alvöru spilun — tveir fjar-spilarar áttuðu sig aldrei á að hægt væri að kveikja á hljóðinu):** hnappurinn („🔇 Unmute to hear the music") var mjó ræma FYRIR NEÐAN sjálfan spilarann — auðvelt að skruna framhjá eða einfaldlega ekki taka eftir, sérstaklega á síma þar sem myndbandið tekur mest af skjánum. Notandi stakk upp á að hafa alla sjálfgefið óþaggaða og frekar leyfa að þagga niður spilarann í staðinn — hafnað (staðfest ástæða þess að fjar-tæki byrja þögguð stendur: mörg vöfrur leyfa alls ekki sjálfvirka spilun MEÐ HLJÓÐI án user-gesture, sjá DuckDuckGo-dæmið að ofan — að byrja óþaggað myndi þýða að myndbandið einfaldlega spilist ALLS EKKI sjálfkrafa hjá mörgum, verra en að þurfa að smella til að heyra). Lausnin sem varð fyrir valinu í staðinn: gera SJÁLFAN „tap to unmute" takkann ómögulegt að missa af. Bæði tilvikin sem áður sýndu sinn eigin, ólíka og lítt áberandi takka (venjulegt „sound off" ástand OG `audioBlocked`-tilvikið þar sem vafri neitar að af-þagga) eru núna EITT sameinað yfirlag (`absolute inset-0`) OFAN Á sjálfu myndbandinu — dökkur bakgrunnur, stórt blikkandi 🔇-tákn (`animate-pulse`) og feitletrað „Tap to unmute", nær yfir alla spilarann svo það er bókstaflega í vegi fyrir að horfa á myndbandið þar til smellt er á það.
...
### Staða — útfært (þriðji hamur: Short = sjálfvirk framvinda þegar allir eru búnir að svara)
Eftir ábendingu frá notanda: Short/Long hamirnir tveir eru núna orðnir ÞRÍR, endurskírðir til að forðast árekstur við eldra gildi sem gæti verið geymt á virku herbergi (`RoundMode` er núna `'auto' | 'timer' | 'full'`, ekki lengur `'short' | 'long'`):
- **„Short"** (`'auto'`, NÝR hamur) — hópurinn heldur áfram í næsta lag um leið og ALLIR sem eiga að giska/gefa einkunn fyrir núverandi lag eru búnir — enginn tímamælir, enginn bíður eftir að lagið klárist. Náttúrulegur endir lagsins (`onEnded`) og handvirkt „Skip song" hjá host virka áfram sem varnet (t.d. ef einhver svarar aldrei), nákvæmlega eins og í hinum hömunum.
- **„Timer"** (`'timer'`) — sami hamur og áður hét „Short": fastur tímamælir (1/90/2 mín), engin breyting á hegðun.
- **„Full Song"** (`'full'`) — sami hamur og áður hét „Long": allt lagið spilast, engin breyting á hegðun.
- **Nöfnin á geymdu gildunum breytt** (ekki bara UI-merkimiðar) af ásettu ráði: að endurnýta strenginn `'short'` fyrir nýja sjálfvirka-framvindu-haminn hefði þýtt að herbergi sem var þegar statt í miðri umferð með `roundMode: 'short'` (gamla merking: fastur tímamælir) myndi við deployment skyndilega fá NÝJU merkinguna án þess að neinn hafi valið hana. Ný `normalizeRoundMode()` í `gameStore.ts` þýðir gömul/óþekkt gildi yfir á nýja týpu-kerfið afturvirkt-öruggt: gamalt `'long'` → `'full'` (sama hegðun), gamalt `'short'` (eða hvað sem er annað/vantar) → `'timer'` (varðveitir gömlu sjálfgefnu hegðunina, ekki nýja sjálfvirka haminn).
- `chooseShortModeCap`/`shortModeCapSeconds`/`SHORT_MODE_CAP_OPTIONS` halda sínum gömlu nöfnum þrátt fyrir endurnefninguna (lýsa enn tímamælinum sem „Timer"-hamurinn notar) — vísvitandi, til að halda breytingunni minni og áhættuminni.
- Nýtt host-only `useEffect` í `05-GuessAndRate.tsx` keyrir `answeredComplete(currentSong)` (fallið er þegar til, notað annars staðar) í hvert sinn sem `guesses` breytist, og kallar `doAdvance()` um leið og allir tilskyldir hafa svarað — sama „advancedForRef" vörn og allir hinir hamirnir nota, svo tvöföld framvinda er útilokuð.
- **Auka lagfæring beðið um í leiðinni**: „X/Y have answered this song" línan var földuð fyrir EIGANDA lagsins sjálfs (`!isOwnSong` skilyrði) — sem þýddi að sá sem á lagið (sá sem hefur mestan áhuga á að vita hversu margir eru búnir að dæma það) sá aldrei töluna, ekki einu sinni host á sínu eigin lagi. Skilyrðið er tekið út (á báðum stöðum sem línan birtist — í sjálfri umferðinni og í eftir-á-breyta-skjánum úr yfirlitinu), talan sjálf þurfti enga breytingu (reiknast nú þegar rétt óháð því hver skoðar).
- Staðfest með lifandi Playwright-prófun (3 spilarar, alvöru `devSubmitGuessAs`-köll ekki bara UI-smellir þar sem það hentaði): Game Setup sýnir Short/Timer/Full Song, tímamælis-undirvalið sést bara undir Timer; í Short-ham framvindar hópurinn EKKI eftir bara eitt af tveimur tilskyldum svörum en gerir það samstundis um leið og bæði eru komin; eigandi lags sér núna „X/Y have answered" á eigin lagi; gömul `'long'`/`'short'` gildi skrifuð beint í gagnagrunninn þýðast rétt yfir í `'full'`/`'timer'`.
...
### Staða — útfært (emoji-viðbrögð við skilaboðum)
- **Gagnalíkan**: `ChatMessage.reactions?: Record<emoji, Record<playerId, true>>` (`types.ts`/`RoomRecord.chat` í `gameStore.ts`) — sama „safn af lyklum sem eru til = satt" mynstur og `lobbyReady`/`finalConfirmations` nota nú þegar, ekki teljari. `toggleChatReaction(messageId, emoji)` les núverandi `chatMessages` úr `get()`, athugar hvort kallandinn eigi nú þegar `reactions/{emoji}/{playerId}`, og skrifar `true` (bæta við) eða `null` (fjarlægja) á nákvæmlega þann eina reit — aldrei endurskrifar restina af skilaboðinu.
- **`src/components/ChatOverlay.tsx`**: fastur listi af 4 emoji (`REACTION_EMOJIS`: 👍 ❤️ 😂 🎉) — nóg til að hylja algeng viðbrögð án þess að þurfa fullan emoji-veljara/leit í svona litlu spjaldi. Undir hverju skilaboði: merki (badge) fyrir hvert emoji sem einhver er þegar búinn að nota (emoji + fjöldi), lituð öðruvísi (`border-primary bg-primary-soft`) ef kallandinn sjálfur er í hópnum sem notaði það — smellur á merki KALLAR EINFALDLEGA `toggleChatReaction` aftur (bætir við sig eða fjarlægir sig af því merki, breytir ekki öðrum). Lítill „+" hringur við hliðina opnar/lokar 4-emoji veljaranum (`pickerForMessageId`, bara eitt skilaboð opið í einu); að velja úr veljaranum kallar `toggleChatReaction` og lokar honum sjálfkrafa.
- Tómt `{}` (allir hafa afturkallað sama viðbragðið) er SÍAÐ út úr birtingunni frekar en eytt úr gagnagrunninum — einfaldara skrifunar-slóð (bara einn reitur breytist í einu), kostar ekkert að sía stakt tómt hlutfall úr við lestur.
- Staðfest með lifandi Playwright-prófun (host + guest): viðbragð frá guest sést samstundis hjá host (fjöldi 1); host bætir sama emoji við, fjöldi verður 2; host afturkallar sitt eigið, fjöldi fer aftur í 1 — allt í rauntíma á milli tveggja tækja, engin endurhleðsla þarf.
...
**Uppfært aftur, eftir ábendingu notanda:** + takkinn birtist núna líka á EIGIN lagi, ekki lengur falinn þar. Upprunalega ástæðan („netagagnslaust að bjarga eigin lagi til sjálfs sín") stóðst ekki í reynd — notandi getur vel viljað eiga sitt eigið val vistað í sinn persónulega lista fyrir næsta skipti (nýtir þá „flýtileið við innsendingu" hér að neðan alveg eins og hvert annað vistað lag). `SaveSongButton` sjálft þurfti enga breytingu (var þegar algjörlega óháð eignarhaldi) — bara `!isOwnSong` skilyrðið sem faldi hnappinn í `05-GuessAndRate.tsx` (báðum stöðum sem `action`-sneiðin er sett) var fjarlægt.
...
### Staða — útfært (eyða úr lista + flýtileið við innsendingu)
Tvö af því sem var frestað hér að ofan eru núna komin:
- **„Remove" takki í `MyListsPanel.tsx`**: hver lína undir opnum flokki fékk rauðan „Remove" takka (sami stíll og „Remove" á vinalista í `FriendsPanel.tsx`) sem eyðir beint úr `users/{uid}/lists/{categoryId}/{songKey}` (`remove()`, engin `.validate`-athugun á eyðingu frekar en annars staðar í appinu). Staðbundna state-ið er uppfært samstundis (ekki endursótt) — heill flokkur hverfur sjálfkrafa úr listanum um leið og síðasta lagið í honum er fjarlægt, sami háttur og `nonEmptyCategories`-síunin virkar nú þegar.
- **Flýtileið við innsendingu (`03-SubmitSong.tsx`)**: `SongForm` sækir núna (ein-skipta `dbGet`, ekki lifandi) `users/{uid}/lists/{category.id}` í hvert sinn sem flokkurinn sem er verið að skila í breytist. Ef eitthvað er vistað þar birtist „Your saved songs for this category" listi FYRIR OFAN leitarformið (bæði eru sýnileg saman — flýtileiðin kemur ekki í staðinn fyrir leitina, enda gæti spilari viljað eitthvað nýtt eða átt ekkert vistað fyrir þennan flokk ennþá) — smellur á „Use this" skilar laginu samstundis (`onSubmit` beint úr vistuðu færslunni), engin leit þarf að eiga sér stað. Staðfestingarspjaldið á eftir endurgerir smámynd úr YouTube's opinbera `i.ytimg.com/vi/{videoId}/default.jpg` slóð (engin API-köllun/kvóti þarf) svo það lítur eins út og þegar lag er valið úr alvöru leit.
- Staðfest með lifandi Playwright-prófun: „Remove" eyðir bæði úr Firebase og staðbundnu state-i, tómur flokkur hverfur sjálfkrafa; flýtileiðin birtist með rétt nafn (úr `youtubeTitle`), „Use this" skilar laginu inn í leikinn með réttu `youtubeVideoId`, og staðfestingarskjárinn sýnir það rétt.

### Staða — útfært (My Lists — sjónræn aðgreining + síðuskipting)
Ábending frá notanda: opna flokka-röðin og lagið undir hana litu of svipað út (`bg-surface-muted` var — og er enn — sama gildi og `bg-surface` í myrkva-þema, svo hausinn og innihaldið undir voru bókstaflega sami litur), og áhyggjur af að langur listi af flokkum yrði endalaus skrun-runa fyrir duglega safnara.
- **Sjónræn aðgreining** (`MyListsPanel.tsx`): opnaða lagalistann er núna sökkt ofan í `bg-bg` (sjálfan síðubakgrunninn, dekkri en `bg-surface-muted` flokka-hausinn) í stað þess að deila sama lit og hausinn fyrir ofan — les núna skýrt sem „innihald undir hausnum" í stað „annar liður í sama lista". Flokka-nafnið er líka orðið `font-semibold` (var `font-medium`) fyrir aukinn stigsmun.
- **Síðuskipting**: `CATEGORIES_PER_PAGE = 6` — bara flokkar með a.m.k. einu vistuðu lagi eru sýndir eftir sem áður, en núna í síðum af 6 í senn með ← / → örvum og „X / Y" teljara fyrir neðan, í stað þess að sýna alla í einu löngu skruni. Örvarnar birtast bara þegar meira en ein síða er til. Núverandi síða er „clömpuð" (`Math.min(page, totalPages - 1)`) svo að eyðing á síðasta lagi á síðustu síðu skilji aldrei eftir tóma síðu í staðinn fyrir að detta sjálfkrafa til baka á síðustu gildu síðuna.
- Staðfest með lifandi Playwright-prófun (innskráður notandi með 8 vistaða flokka): síða 1 sýnir 6 flokka + „1 / 2", síða 2 sýnir hina 2 + óvirkan „næsta" takka; opnaður flokkur sýnir laglistann greinilega dekkri en hausinn fyrir ofan.
...
- **Uppfært eftir ábendingu notanda (skjáskot af „Continue to Scoreboard" undir tilnefningaspjaldinu — takkinn sagði rangt, af því hann fór í raun í stats-spjaldið næst, ekki stigatöfluna)**: í fyrstu var takkinn bara endurnefndur („See Your Stats →"), en notandinn ákvað svo að tilnefningaspjöldin og leiktölfræði-spjaldið eigi einfaldlega heima á SÖMU skjámynd — bæði eru „tölfræði á sinn hátt". `finalStep` er núna bara `'scoreboard' | 'winner' | 'nominations'` (þriðja gildið `'stats'` alveg tekið út) — `'nominations'`-þrepið birtir núna `FinalScoretableCards` (enga eigin takka lengur, hrein flettivél) OG `GameStatsCard` (líka takkalaus núna) STAFLAÐ Á SAMA SKJÁ, með EINUM sameiginlegum „Continue to Scoreboard" takka fyrir neðan bæði í `06-Results.tsx` sjálfu.
- **Aðlögunarlota eftir lifandi skoðun** (nákvæmlega sú lota sem var boðuð hér að ofan): sunburst-ið endurhannað sem `mask` (`repeating-conic-gradient`) OFAN Á `radial-gradient(hvítt → gull/appelsínugult → gagnsætt)` í staðinn fyrir flatan hvítan lit — hver geisli er núna skær hvítur næst kortinu og gulnar/dofnar út á við, auk þess mjórri og fleiri (`1.2deg` breiðir, `8deg` millibil). Tímasetning tveggja laga (sunburst + litaði hringurinn) endurgerð með fjölda keyframe-punkta (`offset`) svo þau nái FULLRI birtu fljótt eftir að snúningnum lýkur („arrive sooner") og HALDIST þar lengi áður en þau dofna alveg í restina, í stað einnar línulegrar stækkunar sem leit út fyrir að vera horfin um leið og hún birtist. „Results" fyrirsögnin ofarlega á skjánum er falin á meðan `finalStep !== 'scoreboard'` (skarst sjónrænt við geislana). Sýnilegi „Continue" takkinn er alveg farinn — spjaldið sjálft er nú `<button>` sem fer beint áfram við snertingu (falið öryggisnet ef hreyfingin klikkar), EN að auki framlengir sig sjálfkrafa (`onContinueRef` í `setTimeout`) ~2 sek eftir að báðar sprengingarnar hafa dofnað alveg út — enginn þarf að snerta neitt í venjulegu falli.
- **`src/logic/winnerFanfare.ts`** (nýtt, eftir ábendingu notanda) — ÞRJÁR tilraunir, í þessari röð:
  1. Tónræn (sveiflugjafar sem spiluðu stutt „fanfare"-stef) — notandi sagði hljóðin einfaldlega „hræðileg".
  2. Tilbúið (synthesized) klapp/fagnaðarlæti úr síuðu hvítu suði — notandi sagði þetta hljóma eins og „rigning eða eitthvað suð", ekki fólk.
  3. **Núverandi lausn**: notandi bjó sjálfur til `public/sounds/` möppu og setti þar inn 8 alvöru `.wav` hljóðbrot (freesound.org-stíl skráarnöfn, t.d. „clapping-hands", „group-of-people-cheering-outside", „congrats") — í stað þess að reyna smíða fagnaðarlæti frekar, spilar appið núna eina af ÞESSUM alvöru skrám, valda af handahófi. Ábyrgð notanda að skrárnar séu heimilar til notkunar (t.d. réttindafrjálst frá Freesound), sama regla og annars staðar í appinu um að hýsa aldrei höfundarréttarvarið efni beint (sjá Spotify/YouTube ákvarðanirnar) — appið sjálft bætir engu höfundarréttarvörðu við, spilar bara það sem er þegar til staðar í `public/`.
  - Ein af þessum 8 skrám (`198305__klankbeeld__and-the-winner-is-male-original.wav`) er 13MB — margfalt stærri en hinar 7 (allar undir 2MB) — og er VÍSVITANDI ÚTILOKUÐ úr slembivalinu (`WINNER_SOUND_FILES` í `winnerFanfare.ts`, athugasemd útskýrir af hverju) frekar en eytt, ef notandi vill nota hana seinna (t.d. eftir að hafa stytt hana). Aðeins VALDA skráin er sótt (`fetch` + `decodeAudioData`, í gegnum sömu `AudioContext` og `primeWinnerFanfare()` undirbjó) — EKKI allar 8 í hvert sinn, til að spara gagnamagn á síma.
  - `playWinnerCheer()` skilar `stop()` sem `WinnerRevealCard`s effect-hreinsun kallar (við hliðina á `spinAnim.cancel()`) — stöðvar `AudioBufferSourceNode`-ið samstundis ef component-ið er tekið úr DOM áður en skráin er búin að spilast (raunveruleg snemmbúin „tap to skip", eða React StrictMode's tvöfalda dev-mode mount/cleanup/mount).
  - Byrjar SAMSTUNDIS þegar spuna-hreyfingin byrjar, spilast svo út sína eigin náttúrulegu lengd óbreytt (þarf ekki að klippa/lengja til að passa við hreyfinguna — þetta eru nú þegar tilbúin, afmörkuð hljóðbrot). Örstutt fade-in (~30ms) til að forðast „klikk" í upphafi.
- Staðfest með lifandi Playwright-prófun: allar 7 leyfðu skrárnar sækjast og afkóðast án villu (þvingað í gegnum hverja að handahófi, ekki bara treyst á tilviljun); raunverulegur UI-smellur á „Final Results" kveikir á alvöru afspilun. Sjálf hljóðgæðin eru huglægt mat sem sjálfvirk próf ná ekki utan um — þarf lifandi hlustun, en þar sem þetta eru núna ALVÖRU hljóðbrot (ekki tilbúin úr grunni) er miklu líklegra að þau hljómi rétt en fyrri tvær tilraunirnar.
...
- **Gagnalíkan**: nýtt `users/{uid}/careerStats` — algjörlega aðskilið frá `players/{playerId}`s room-sértæku uppsöfnuðu teljurunum (sem núllstillast við `startNewGame`). Inniheldur: `roundsPlayed`, `gamesPlayed`, `totalCorrectGuesses`, `totalGuessAttempts` (fyrir nákvæmnihlutfall), `totalOwnedSongCount`/`totalGuessedByOthersCount` (fyrir „hversu erfitt er að lesa þig"), `ratingGivenSum`/`ratingGivenCount` og `ratingReceivedSum`/`ratingReceivedCount` (tvær aðskildar meðaleinkunnir eins og beðið var um), `currentGuessStreak`/`longestGuessStreak`, og `titleCounts` (hversu oft hver af fimm titlunum + „Game Winner" hefur unnist).
- **Mikilvæg arkitektúr-uppgötvun**: það er engin leið að láta EITT tæki skrifa uppsafnaða tölfræði fyrir ALLA spilara í herberginu (eins og `applyRoundScoresIfNeeded` gerir fyrir room-sértæku teljarana) — engar Cloud Functions eru í þessu verkefni, og Firebase-reglurnar geta bara heimilað skrif fyrir þann `uid` sem er í alvöru innskráður á því tæki sem sendir skrifið. Þess í stað foldar HVERT innskráð tæki sína EIGIN account-tölfræði sjálft, kallað úr `returnToLobby()` (`gameStore.ts`s nýja `applyCareerStatsIfNeeded()`) — sama kall-staður og allir spilarar nota nú þegar, bæði mitt í leik og í leikslok.
- **Fundinn og lagaður raunverulegur böggur í eigin prófun sama kvöld**: `returnToLobby()` er ekki varið gegn því að vera kallað aftur eftir að umferð/leikur er þegar búinn (t.d. „til baka" í vafra á gamla Results-skjáinn eftir að `finalizeRoundIfReady` er þegar búið að núllstilla og senda hópinn í Lobby) — ólíkt room-sértæka foldinu, sem er náttúrulega skaðlaust við endurtekið kall (umferðargögnin eru þá tóm, svo allar upphæðir reiknast 0), hefði endurtekið kall á `applyCareerStatsIfNeeded()` tvöfaldað `roundsPlayed`/`gamesPlayed`/`titleCounts` í alvörunni, af því þau lesa `players`-fylkið sem geymir gögn sem ALDREI tæmast milli umferða. Lagað með einfaldri vörn: fallið byrjar á `if (phase !== 'results') return` — ósvikið kall gerist alltaf meðan Results-skjárinn fyrir þessa umferð er ennþá uppi (`finalizeRoundIfReady` getur ekki verið búið að breyta phase, því það krefst þess að ÞETTA tæki sé sjálft þegar orðið `lobbyReady`, sem gerist ekki fyrr en rétt á eftir), en endurtekið/villt kall lendir alltaf á að herbergið er þegar farið í `'lobby'`.
- **Engin breyting þurfti á Firebase-reglunum**: staðfest með beinni prófun (skrifun í `users/{uid}/careerStats/*` gekk strax án `permission_denied`) — `.write`-reglan á `$uid` sjálfu (owner-only, sett upp fyrir `lists`) cascade-ar sjálfkrafa niður á hvaða nýja systurgrein sem er, nákvæmlega eins og Firebase's cascade-hegðun sem kom í ljós sem böggur annars staðar í þessum kafla.
- **`src/logic/scoring.ts`**: þrjú ný hrein föll — `countGuessAttempts`, `ratingsGivenStats` (spegilmynd af núverandi `ownSongRatingStats`, bara frá gefandans hlið), `guessSequenceForPlayer` (röð rétt/rangt fyrir giskin sem þessi spilari gerði, í spilunarröð — notað fyrir runu-útreikninginn).
- **`src/components/StatsPanel.tsx`** (nýtt, kom í staðinn fyrir „coming soon" stubbinn) — sama ein-skots-sóknarmynstur og `MyListsPanel.tsx` (`dbGet` við mount, engin lifandi tenging). Sýnir vinaleg „Play a full game while signed in..." skilaboð ef `gamesPlayed` er 0, annars alla töluna + titlana sem hafa unnist.
- Staðfest með beinni Playwright-prófun (2 umferðir, einn innskráður + einn nafnlaus spilari): réttar tölur í `users/{uid}/careerStats` eftir umferð 1 OG eftir leikslok (umferð 2), nafnlaus spilari fær ENGA skrifun, endurtekið `returnToLobby()`-kall breytir engu (staðfestir `phase`-vörnina að ofan), og Stats-flipinn í UI-inu sýnir nákvæmlega sömu tölur og REST-lesturinn.

### Staða — útfært (verðlaunaspjöld skipt út fyrir hönnuð badge-myndir)
Notandi bjó til `public/badges/` með sex fullbúnum JPG-spjöldum (eitt per titil + „Round Winner" fyrir heildarsigurvegarann) — hvert þeirra er nú þegar heil hönnuð eining (litaður rammi, tákn, titill OG undirtexti bakað inn í myndina sjálfa), ekki bara hrátt tákn. `AwardCard.tsx` skipt algjörlega út: gömlu innfelldu SVG-táknin (og eyðubrún/titil/undirtexti sem appið teiknaði sjálft ofan á þau) eru farin — `AwardCard` sýnir núna bara viðeigandi badge-mynd (`BADGE_IMAGE`-uppfletting eftir `AwardIconKind`, sama munstur og `WINNER_SOUND_FILES` í `winnerFanfare.ts`) plús nafn sigurvegarans/sigurvegaranna fyrir neðan — það eina sem myndirnar geta ekki innihaldið sjálfar. `title`/`subtitle` proppin haldast á `AwardCardProps` (kallendur senda þau enn) en eru ekki lengur teiknuð — hefði bara verið endurtekning á öðruvísi orðuðum texta sem myndin sjálf sýnir nú þegar.
- **„Round Winner" badge-inn** notaður fyrir `'overall-winner'` — af því `WinnerRevealCard` vefur `AwardCard` almennt (ekki sértækt fyrir gamla SVG-táknið), fær nýja myndin sjálfkrafa sömu spuna+ljósgeisla-hreyfingu og áður án nokkurrar viðbótarvinnu.
- **Þekkt málamiðlun**: JPG-skrárnar hafa ógegnsæjan, nær-svartan bakgrunn utan um sinn eigin ávala ramma (ekkert alfa-gagnsæi) — `overflow-hidden rounded-2xl` á umgjörðinni mýkir ferköntuðu ytri hornin, og í myrkva-þema blandast þetta óaðfinnanlega við restina af appinu. Í ljósa-þema sést spjaldið sem dökkur ferhyrningur á ljósum bakgrunni — læsilegt en sjónrænt ekki fullkomlega samræmt. Ekki lagað (þyrfti gegnsæjar PNG-myndir í staðinn), flaggað fyrir notanda til að meta.
- `music_mind_reader_badges_all.jpg` (samsett yfirlitsmynd sem notandi deildi til viðmiðunar, sýnir öll sex spjöldin saman) er vísvitandi EKKI hluti af `public/` í git-inu — appið notar hana aldrei, sama regla og 13MB hljóðskráin sem var skilin eftir ónotuð í „Leik-lokaflæði" kaflanum.
...
- **Uppfært**: notandi endurnefndi „Round Winner" í **„Game Winner"** (réttara heiti — spjaldið er fyrir HEILDARSIGURVEGARA leiksins, ekki bara umferðar) og lagaði undirtextann í leiðinni („Highest combined score across all rounds."), auk þess að fínpússa **Musical Criminal** badge-ið („Guilty of the lowest-rated song."). Bæði skil sem ný skráarnöfn (`game-winner-card-v3-corrected.png`, `musical-criminal-v3.png`, PNG núna í stað JPG) — `BADGE_IMAGE`-uppflettingin í `AwardCard.tsx` uppfærð til að vísa á þau, gömlu skrárnar (`round_winner.jpg`/`musical_criminal.jpg`) fjarlægðar. Staðfest með lifandi prófun: Winner reveal sýnir „GAME WINNER" með fullri hreyfingu, Musical Criminal birtist rétt í tilnefningaþokknum.
...
### Staða — útfært (hljóðstyrks-stilling eftir lifandi hlustun + böggur fannst)
- **Hljóðstyrkur lækkaður fjórum sinnum** eftir endurgjöf notanda á meðan á alvöru spilun stóð: full styrkur → 0.35 („allt of hátt") → 0.12 → 0.1 → **0.08** (núverandi, `MAX_VOLUME` í `backgroundMusic.ts`).
- **Per-laga leiðrétting** (`VOLUME_MULTIPLIER`): „The Mountain – Retro Game" var áberandi hærra en hin lögin (Pixabay-lögin koma frá ólíkum höfundum, ekki hljóðstyrks-samræmd sín á milli) — fékk `0.5` margföldunarstuðul ofan á `MAX_VOLUME`, þ.e. helmingi lægra en hin. `targetVolume()` les núna `currentFile` til að fletta upp margfaldara, sjálfgefið `1` (engin breyting) fyrir öll ónefnd lög.
- **Alvöru böggur fannst og lagaður**: notandi tók eftir að bara ~3 af 6 lögum heyrðust nokkurn tímann eftir margar (~20) leiki. Rótin: `vadim_makes_sound_quiz-...` (undirstrik) í handskráða listanum passaði EKKI við alvöru skráarnafnið í Storage-fötunni, sem er `vadim_makes_sound-quiz-...` (bandstrik) — staðfest með beinni REST-fyrirspurn á `firebasestorage.googleapis.com`s lista-API (`GET /v0/b/{bucket}/o`) sem sýndi nákvæmu skráarnöfnin. Þetta eina rangstafaða nafn skilaði 404 í hvert sinn sem það var valið af handahófi, sem `enterLobby()`s `try/catch` gleypti hljóðalaust (rétt hönnun — tónlist á aldrei að geta brotið leikinn — en falda þar með bilunina þar til notandi tók eftir mynstrinu sjálfur). Lagað með því að leiðrétta stafsetninguna í `BACKGROUND_MUSIC_FILES`.

### Staða — útfært (tónlist bara hjá host — nema í fjarspilun)
Ábending frá notanda: ef allir eru saman í sama herbergi og HVERT tæki spilar sína eigin slembivöldu bakgrunnstónlist óháð hvort öðru, verður útkoman margar ósamstilltar tónlistarlínur ofan í hvor aðra úr mismunandi símahátölurum — ekki það sem til stóð. Lausnin: tónlist er núna bundin við HOST-tækið eitt í venjulegri spilun (allir í sama herbergi, host-síminn er nógu hátt til að heyrast), en um leið og fjarspilun (`remotePlayEnabled`, sjá „Textaspjall fyrir fjarspilun" kaflann) er kveikt, fá ÖLL tæki sína eigin tónlist — enda er þá einhver ekki í sama rými og þarf sinn eigin hátalara — og geta samt mute-að sig sjálf með takkanum sem er þegar til.
- **`useBackgroundMusic` í `App.tsx`**: nýtt `eligible = isHost || remotePlayEnabled` skilyrði bætt við hlið fasa-rökréttarinnar. Óhæft tæki (ekki host, fjarspilun slökkt) keyrir ALDREI `enterLobby()`/`resumePlaying()` — ekkert `<audio>`-element er einu sinni búið til hjá því. „Nýbyrjun"-rökréttin sem áður bara náði yfir „ný stofa" er núna líka virkjuð þegar tæki verður HÆFT í fyrsta sinn (fjarspilun kveikt á meðan einhver er í `setup`/`results`) — sami háttur og fersk stofa fær sitt fyrsta lag.
- Staðfest með lifandi Playwright-prófun (host + gestur): gestur fær ALDREI `<audio>`-element á meðan fjarspilun er slökkt; um leið og host kveikir á fjarspilun byrjar tónlist samstundis hjá gestinum (rétt lag + rétt hljóðstyrkur, þ.m.t. per-laga leiðréttingin); slökkt aftur á fjarspilun → tónlist gestsins mýkist niður og stoppar; host-tækið algjörlega ósnert allan tímann.

### Staða — útfært (tvö aðskilin lagasöfn: Lobby og Scorboard)
Notandi endurskipulagði `public`-fötuna í Storage í tvær möppur: **`Lobby/`** (þrjú lög, notuð fyrir lobby+setup) og **`Scorboard/`** (tvö NÝ lög — `gr0za-upbeat-upbeat-music-596488.mp3`, `sonican-quiz-background-loop-thinking-news-275636.mp3` — notuð sérstaklega meðan fólk er að skoða stig/tölfræði). Í leiðinni fækkaði Lobby-safninu líka úr 6 í 3 lög (`aurectheme-cocktail-jazz`/`andriih-funny-funny-music`/`vadim_makes_sound-quiz...` tekin út).

- **Tvö aðskilin söfn** (`backgroundMusic.ts`): `LOBBY_MUSIC_FILES`/`SCOREBOARD_MUSIC_FILES` (bæði með `Lobby/`/`Scorboard/` forskeyti í skráarnöfnunum sjálfum, enda er það alvöru slóðin í Storage núna). Sameiginlegt `pickAndPlay(files)` hjálparfall (var `enterLobby()` beint áður) tekur núna við fylkinu sem á að velja úr — `enterLobby()`/nýtt `enterScoreboard()` eru þunnar sérhæfingar ofan á það.
- **„Hópur" (group) í stað hrárrar fasa-rökréttar** (`useBackgroundMusic` í `App.tsx`): `musicGroupForPhase()` flokkar hverja fasa í `'lobby'` (lobby+setup), `'scoreboard'` (results) eða `'silent'` (submit+guess). Nýtt lag er BARA valið þegar HÓPURINN sjálfur breytist (t.d. silent→lobby, eða lobby→scoreboard) — að fara á milli lobby og setup (sami hópur) heldur bara áfram sama laginu í stað þess að stokka upp á nýtt, ólíkt því sem gerðist áður þegar hvert „ný lobby" tilvik alltaf endurvaldi.
- **Aukalagfæring fannst í leiðinni**: mute-áskriftin (`useMusicStore.subscribe`) notaði áður `MAX_VOLUME` beint í stað `targetVolume()` þegar kveikt var aftur á hljóði — þýddi að lag með per-laga afslátt (t.d. „The Mountain") hefði stokkið í FULLAN `MAX_VOLUME` í stað síns eigin lægri marks við afmute. Lagað til að nota `targetVolume()` alls staðar.
- Staðfest með lifandi Playwright-prófun: lobby+setup halda sama lagi (Lobby-safn); submit/guess þagna eðlilega; results skiptir yfir í Scorboard-safn með nýju lagi og réttum hljóðstyrk. Engin breyting þurfti á Storage-reglum (`{allPaths=**}` nær yfir undirmöppur nú þegar).

### Staða — útfært (+ takkinn virkjaður á eigin lagi líka)
Upprunalega ákvörðunin um að fela + takkann á eigin lagi („Notendaaðgangur..." kaflinn: „netagagnslaust að bjarga eigin lagi til sjálfs sín") stóðst ekki í reynd — notandi getur vel viljað eiga sitt eigið val vistað fyrir næsta skipti. `SaveSongButton` sjálft þurfti enga breytingu (var þegar algjörlega óháð eignarhaldi laga); `!isOwnSong` skilyrðið sem faldi hnappinn var fjarlægt á báðum stöðum í `05-GuessAndRate.tsx` (mitt í umferð og í yfirlits-breytingarskjánum). Staðfest með lifandi prófun: + birtist núna á eigin lagi, vistun virkar eins og fyrir hvert annað lag.